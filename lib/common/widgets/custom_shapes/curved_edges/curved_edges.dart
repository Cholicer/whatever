...
class TCustomCurvedEdges extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(firstStartCurve.dx, firstStartCurve.dy, firstEndCurve.dx, firstEndCurve.dy);

    final secondStartCurve = Offset(0, size.height - 20);
    final secondEndCurve = Offset(size.width - 30, size.height - 20);
    path.quadraticBezierTo(secondStartCurve.dx, secondStartCurve.dy, secondEndCurve.dx, secondEndCurve.dy);

    final thirdStartCurve = Offset(size.width, size.height - 20);
    final thirdEndCurve = Offset(size.width, size.height);
    path.quadraticBezierTo(thirdStartCurve.dx, thirdStartCurve.dy, thirdEndCurve.dx, thirdEndCurve.dy);
